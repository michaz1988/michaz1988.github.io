...
def create_listitems(data=None):
    ...
    for count, result in enumerate(data):
        ...
        for key, value in result.items():
            ...
            if key.lower() in ["label"]: listitem.setLabel(value)
            elif key.lower() in ["search_string"]:
                path = f"plugin://plugin.program.autocompletion/?info=selectautocomplete&id={quote(value)}"
...
if __name__ == "__main__":
    ...
    if "info" in params: infos.append(params['info'])
    if infos: start_info_actions(infos, params)
