...
provider_temppath = os.path.join(temppath, "tvdDE")
...
## Channel Files
tvdDE_chlist_selected = os.path.join(datapath, 'chlist_tvdDE_selected.json')
...
def get_channellist():
	...
	for channels in tvdDE_channels: temp.append({"contentId": channels['id'],"name": channels['name'],"pictures": [{"href": "https://sender.epglogos.de/240x200/"+ channels['img']}]})
...
def select_channels():
	...
	if not os.path.isfile(tvdDE_chlist_selected):
		with open((tvdDE_chlist_selected), 'w', encoding='utf-8') as selected_list:
			selected_list.write(json.dumps({"channellist": []}))
	## Download chlist_tvdDE_provider.json
	provider_list = get_channellist()
	dialog = xbmcgui.Dialog()
	with open(tvdDE_chlist_selected, 'r', encoding='utf-8') as s:
	...
	if user_select is not None:
		with open(tvdDE_chlist_selected, 'w', encoding='utf-8') as f:
			json.dump(user_select, f, indent=4)
		if os.path.isfile(tvdDE_chlist_selected):
			...
			if valid is True:
				...
				if yn: select_channels()
					...
					xbmcvfs.delete(tvdDE_chlist_selected)
					exit()
	else:
		valid = check_selected_list()
		if valid is True:
...
def check_selected_list():
	check = 'invalid'
	with open(tvdDE_chlist_selected, 'r', encoding='utf-8') as c:
...
# Channel Selector
try:
	if sys.argv[1] == 'select_channels_tvdDE': select_channels()
